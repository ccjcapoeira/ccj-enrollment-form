...
2. シート（タブ）名は次の 3 つを推奨。**存在しなくても、初回送信時に GAS が自動作成しヘッダーも自己補完します。**
   - `入会`（入会申し込み, 18 列）
   - `プラン変更`（プラン変更 / 休会希望, 11 列）
   - `退会手続き`（退会のお手続き, 8 列）
3. 手動でヘッダーを用意する場合は、`入会` の 1 行目を次の順で入れてください（GAS の書き込み順と 1:1 で対応）。
...
| A | B | C | D | E | F | G | H |
|---|---|---|---|---|---|---|---|
| 申請日時 | 氏名 | メールアドレス | 電話番号 | 退会希望月 | 最終参加予定日 | 退会理由 | 備考 |

> 旧フォーム時代に 12 列で作られたシートがあると、新しい送信内容が右にずれて記録されます。その場合は管理用スクリプト `ccj-member-tools/realign_enrollment_sheet.py` で 18 列に再整列できます（実運用シートに対しては実行前に必ずバックアップを取ってください）。
...
- 既存の「プラン変更」「退会手続き」「入会」シートにヘッダー欠落があれば、GAS が初回送信時に自動で足します（`ensureHeaders_`）。ただし列順が GAS の期待順と異なる場合は新規列が末尾に追加されるため、列順を直すならシート側で並べ替えてください
- 入会用データは、シート名が **入会** のタブに書き込みます。タブが無ければ GAS が「入会」を自動作成しヘッダーを書き出します
